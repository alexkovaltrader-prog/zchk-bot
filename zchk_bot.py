...
TRACKS = {
    ...
    "beginner": {
        ...
        "result": {
            ...
            "offer": "ZCHK даёт то чего не хватает — систему. Риск-менеджмент, чтение рынка через нарратив, психология под давлением. Наша система не просто даёт мимолётный результат, она работает на любой фазе рынка и на любом активе за счет мульти-системы управления рисков. Один слитый депозит стоит дороже курса.\n\nДо 30 июня триал бесплатно, чтобы ознакомиться с платформой."
        }
    },
    "experienced": {
        ...
        "result": {
            ...
            "offer": "ZCHK — целостная система от структуры рынка до психологии решений. Не с нуля, а поверх твоего опыта. Плюс адаптация под проп-фирмы и торговлю личного счёта.\n\nДо 30 июня триал бесплатно. Регистрация за 30 секунд."
        }
    }
}
...
async def _handle_menu_button(query, context, user, data):
    ...
    if data == "show_story":
        ...
        # Фото Ярослава отдельно, текст истории отдельно
        photo_yar = await fetch_photo(f"{GITHUB_BASE}/IMG_0101.JPG")
        if photo_yar:
            try:
                await context.bot.send_photo(chat_id=chat_id, photo=photo_yar)
            except Exception as e:
