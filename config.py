...
WORK_ACCOUNT_URL = (os.getenv("WORK_ACCOUNT_URL") or "").strip()
PROP_VIDEO_URL = (os.getenv("PROP_VIDEO_URL") or "").strip()
PROP_VIDEO_ENABLED = False
# Дубль экрана 5. False = скрыть без правки воронки.
SCREEN_S5_TWO_WEEKS_ENABLED = False

# Тарифы academy profiles.plan -> ветка дожима. Не покупатель не должен попасть в C/D.
PLAN_QUICK = frozenset({"book"})
PLAN_FULL = frozenset({"book_video", "video", "extended"})
